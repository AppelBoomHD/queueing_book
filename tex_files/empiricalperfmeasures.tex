...
For the second case---which I found quite hard, and several of my attempts were wrong at first---observe that we can interpret this as a queue with `virtual' loss.
As an example, suppose that $m=5$, $a_k = 7$, and $L_k=9$.
If we would not accept the jobs in excess of $m=5$, then $L_k-m = 4$ would be lost of the $7$ that arrived.
So, let us call $v_k = \max\{L_k-m, 0\}$ the virtual loss.
Then, $w_k = \max{a_k-v_k, 0}$ are the jobs that are `virtually' accepted.
In other words, $w_k$ jobs see $m$ or less in the system. 
