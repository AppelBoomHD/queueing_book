...
When the demand is finite, the numbers $\{\pi(k)\}$ should decrease geometrically fast for all $k\geq N$ where $N$
is some large number\footnote{An interesting question, why should it decrease monotonically after some, large, $N$?}.
...
\end{equation*}
Observe that this is in line with~\cref{ex:57}.
\begin{solution}
...
p = np.ones(num)
for n in range(1, num):
    p[n] = sum(p[n - 1 - m] * G[m] for m in range(min(n, len(G))))

p *= labda / mu
p /= p.sum()  # normalize
\end{pyconsole}
Here we use a small trick to speed up the computations.
Note that $\sum_{m=0}^n \pi(m)G(n-m) = \sum_{m=0}^n \pi(n-m) G(m)$.
We also know that $G(m)=0$ for $m$ sufficiently large; here this happens for sure when \texttt{m >= len(G)}.
Thus, we don't have to evaluate the summation up to $n$, but we can stop once we reach \texttt{len(G)}.
Another trick is to take the multiplication $\lambda/\mu$ out of the for-loop.
