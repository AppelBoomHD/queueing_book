...
The first set of exercises are somewhat technical; we ask you to derive the formulas---the main challenge is not to make computational errors.
...
\end{equation*}
where we use that $A_{A(t)+1}$ is the arrival time of the next job after time $t$.
...
Service time is sojourn time minus time in queue.
\begin{pyconsole}
ES = W - Wq
...
The expected number of jobs at the server is the expected number in the system minus the the expected number in queue. Thus, 
\begin{pyconsole}
rho = EL - ELq
rho
\end{pyconsole}
...
all $m$ servers are idle. If the system contains one customer, $1$
server is busy, and so on. We therefore define the \emph{utilization of the station}  as
\begin{equation*}
u = \sum_{n=0}^\infty \min\{n,m\} p_n.
\end{equation*}


\begin{pyconsole}
u = sum(min(n, m) * P[n] for n in range(len(P)))
u
\end{pyconsole}
...
\begin{exercise}[Hall 5.3]\clabel{ex:l-823}
...
The load per server is given by:
\begin{pyconsole}
rho = labda * ES/c
rho
\end{pyconsole}
...
For the utilization, see  \cref{exer: Hall}. We now have $c=2x$ servers.
\begin{pyconsole}
u = sum(min(n, c) * P[n] for n in range(len(P)))
u
\end{pyconsole}
And this matches with the above. 
\end{solution}
\end{exercise} 
